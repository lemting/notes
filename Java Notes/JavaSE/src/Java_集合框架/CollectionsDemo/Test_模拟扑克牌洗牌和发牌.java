...
public class Test_模拟扑克牌洗牌和发牌 {
	...
	public static void test2() {
		...
		for(int i = list3.size() - 1;i >= 0;i--) {
			if(i == list3.size() - 1)
				System.out.print("[");
			System.out.print(map.get(list3.get(i)));
			if(i != 0)
				System.out.print(", ");
			else 
				System.out.println("]");
		...
		for(int i = list4.size() - 1;i >= 0;i--) {
			if(i == list4.size() - 1)
				System.out.print("[");
			System.out.print(map.get(list4.get(i)));
			if(i != 0)
				System.out.print(", ");
			else 
				System.out.println("]");
	...
	public static void doudizhu() {
		...
		//发牌
		for(int i = 0;i < list.size();i++) {	
			if(i < list.size()-3) { //3个玩家的牌
				switch(i%3) {
					case 1:	list1.add(list.get(i));		break;
					case 2: list2.add(list.get(i));		break;
					case 0: list3.add(list.get(i));		break;	
				}
			} else { //最后3张底牌
				list4.add(list.get(i));
			}
		}
		//通过Collections类中的sort()方法对对3个List对象排序
